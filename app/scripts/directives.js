	...
	return {
		...
		link: function (scope, element, attrs) {
			var ctx = element[0].getContext('2d'),
				...
				chart = new Chart(ctx),
				myChart = chart.Bar(scope.chart.data, options);

			scope.chart.update = function () {
				myChart.destroy();
				chart.Bar(scope.chart.data, options);
			};
	...
	return {
		...
		link: function (scope, element, attrs) {
			var ctx = element[0].getContext('2d'),
				...
				chart = new Chart(ctx),
				myChart = chart.Line(scope.chart.data, options);
	...
	return {
		...
		link: function (scope, element, attrs) {
			var ctx = element[0].getContext('2d'),
				data = [
				    {
				        value: scope.pie.value,
				        color:"#D7A03D",
				        label: scope.pie.unit
				    },
				    {
				        value: 150-scope.pie.value,
				        color: "#D6D6D6",
				        label: "Goal"
				    }
				],
